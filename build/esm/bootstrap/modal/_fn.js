...
export const handleUpdate = (elem) => {
...
export const show = (elem, relatedTarget) => {
    if (isTag(elem)) {
        ...
        if (mdl) {
            addEvent("hidden.bs.modal", mdl, (e) => {
...
export const Create = (attr) => {
    ...
    if (attr.customStyle === 1) {
        ...
        contAttr.view = "end";
        return new container(contAttr, [
            showHeader
                ? new header(mergeAttr({
                    borderNone: "bottom",
                    close: attr.backdrop === "static" ? false : btn.hasDismissButton,
                }, attr.attrHeader), new title(attr.title || document.title))
                : "",
            new body(mergeAttr({ paddingY: 0 }, attr.attrBody), attr.elem ? attr.elem : ""),
            showFooter
                ? new footer(mergeAttr({
                    flex: "column",
                    alignItem: "stretch",
                    width: 100,
                    gap: 2,
        ...
        return new container(contAttr, [
            showHeader
                ...
                : "",
            new body(mergeAttr({ padding: 4, paddingTop: showHeader ? 0 : undefined, textAlign: "center" }, attr.attrBody), attr.elem ? attr.elem : ""),
            showFooter
                ? new footer(mergeAttr({
                    flex: ["nowrap", "grow-1"],
                    padding: 0,
