...
export class bsClassFormatterRule {
    shared;
    value;
    format;
    formatValue;
...
})(attachBSClass || (attachBSClass = {}));
let allowAttrProp = [];
export var attachBSAttr;
...
export const bsConstArg = (prop, arg) => {
    if (arg) {
        if (arg.length === 1) {
            if (isAttr(arg[0])) {
                return arg[0];
...
export const bsConstArgTag = (prop, t1, t2, fn, arg) => {
    if (arg) {
        if (arg.length === 1) {
            if (isAttr(arg[0])) {
            ...
            return fn(mergeObject({ [prop]: arg[1] }, arg[0])) ? t2 : t1;
        }
        else {
            return t1;
        }
