"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Create = exports.show = exports.hide = exports.dispose = exports.toggle = exports.handleUpdate = exports.getOrCreateInstance = exports.getInstance = exports.init = void 0;
const builder_js_1 = require("../../core/builder.js");
const eventManager_js_1 = require("../../core/eventManager.js");
const mergeAttr_js_1 = require("../../core/mergeAttr.js");
const tag_js_1 = require("../../core/tag.js");
const uuid_js_1 = require("../../core/uuid.js");
const button_js_1 = require("../button.js");
const body_js_1 = require("./body.js");
const container_js_1 = require("./container.js");
const footer_js_1 = require("./footer.js");
const header_js_1 = require("./header.js");
const title_js_1 = require("./title.js");
const btnclose_js_1 = require("./btnclose.js");
const console_js_1 = require("../../core/console.js");
const bootstrap_1 = require("bootstrap");
// import { disconnectResizeObserver, observeResizeObserver } from "../../core/resizeObserverManager.js";
const init = (elem, options) => {
    return (0, exports.getOrCreateInstance)(elem, options);
};
exports.init = init;
const getInstance = (elem) => {
    return bootstrap_1.Modal.getInstance(elem);
};
exports.getInstance = getInstance;
const getOrCreateInstance = (elem, options) => {
    (0, eventManager_js_1.addEvent)("destroy", elem, (i) => {
        const target = i.target;
        // disconnectResizeObserver(target);
        const m = (0, exports.getInstance)(target);
        if (m) {
            console_js_1.bstsConsole.info(`Dispose bootstrap modal from $1`, target);
            m.hide();
            m.dispose();
        }
    });
    // observeResizeObserver(elem, (r) => {
    // 	handleUpdate(elem);
    // });
    console_js_1.bstsConsole.info(`Initialize bootstrap modal to $1`, elem);
    return bootstrap_1.Modal.getOrCreateInstance(elem, options);
};
exports.getOrCreateInstance = getOrCreateInstance;
const handleUpdate = (elem) => {
    (0, exports.getOrCreateInstance)(elem)?.handleUpdate();
};
exports.handleUpdate = handleUpdate;
const toggle = (elem, relatedTarget) => {
    (0, exports.getOrCreateInstance)(elem)?.toggle(relatedTarget);
};
exports.toggle = toggle;
const dispose = (elem) => {
    (0, exports.getOrCreateInstance)(elem)?.dispose();
};
exports.dispose = dispose;
const hide = (elem) => {
    if (typeof elem === "string") {
        (0, exports.getOrCreateInstance)(elem)?.hide();
    }
    else {
        const mdl = elem.classList.contains("modal") ? elem : elem.closest(".modal");
        if (mdl) {
            (0, exports.getOrCreateInstance)(mdl)?.hide();
        }
    }
};
exports.hide = hide;
const show = (elem, relatedTarget) => {
    if ((0, tag_js_1.isTag)(elem)) {
        if (!elem.attr) {
            elem.attr = {};
        }
        elem.attr.id ??= (0, uuid_js_1.UUID)();
        (0, builder_js_1.appendChild)(document.body, elem);
        let mdl = document.getElementById(elem.attr.id);
        if (mdl) {
            (0, eventManager_js_1.addEvent)("hidden.bs.modal", mdl, (e) => {
                (0, builder_js_1.removeElement)(e.target);
            });
            (0, exports.getOrCreateInstance)(mdl).show(relatedTarget);
        }
    }
    else {
        (0, exports.getOrCreateInstance)(elem)?.show(relatedTarget);
    }
};
exports.show = show;
const genBtnItem = (customStyle, btn) => {
    if (btn) {
        if (!Array.isArray(btn)) {
            btn = [btn];
        }
        if (customStyle === 1) {
            return btn.map((i) => {
                if (typeof i.click === "function") {
                    return new button_js_1.button({
                        weight: "lg",
                        color: i.color ? (i.color === "transparent" ? "secondary" : i.color) : undefined,
                        on: { click: i.click },
                    }, i.elem);
                }
                else {
                    return new button_js_1.button({
                        weight: "lg",
                        color: i.color ? (i.color === "transparent" ? "secondary" : i.color) : undefined,
                        dismiss: "modal",
                    }, i.elem);
                }
            });
        }
        else if (customStyle === 2) {
            const lastBtnIndex = btn.length - 1;
            return btn.map((i, ix) => {
                if (typeof i.click === "function") {
                    return new button_js_1.button({
                        fontWeight: i.color ? (i.color !== "transparent" ? "bold" : undefined) : undefined,
                        border: ix < lastBtnIndex ? "end" : undefined,
                        flex: ["grow-1", "wrap"],
                        textDecoration: "none",
                        textColor: i.color !== "transparent" ? i.color : "primary",
                        color: "transparent",
                        rounded: 0,
                        weight: "lg",
                        fontSize: 6,
                        paddingY: 3,
                        margin: 0,
                        on: { click: i.click },
                    }, i.elem);
                }
                else {
                    return new button_js_1.button({
                        fontWeight: i.color ? (i.color !== "transparent" ? "bold" : undefined) : undefined,
                        border: ix < lastBtnIndex ? "end" : undefined,
                        flex: ["grow-1", "wrap"],
                        textDecoration: "none",
                        textColor: i.color !== "transparent" ? i.color : "primary",
                        color: "transparent",
                        rounded: 0,
                        weight: "lg",
                        fontSize: 6,
                        paddingY: 3,
                        margin: 0,
                        dismiss: "modal",
                    }, i.elem);
                }
            });
        }
        else {
            return btn
                .map((i) => {
                if (typeof i.click === "function") {
                    return new button_js_1.button({
                        color: i.color,
                        on: { click: i.click },
                    }, i.elem);
                }
                else {
                    return new button_js_1.button({
                        color: i.color,
                        dismiss: "modal",
                    }, i.elem);
                }
            })
                .reverse();
        }
    }
    else {
        return [];
    }
};
const btnTypeDB = (btnType) => {
    switch (btnType) {
        case "ok":
            return {
                color: "primary",
                elem: "Okay",
            };
        case "cancel":
            return {
                color: "transparent",
                elem: "Cancel",
            };
        case "close":
            return {
                color: "transparent",
                elem: "Close",
            };
        case "yes":
            return {
                color: "primary",
                elem: "Yes",
            };
        case "no":
            return {
                color: "transparent",
                elem: "No",
            };
        case "retry":
            return {
                color: "primary",
                elem: "Retry",
            };
        case "continue":
            return {
                color: "primary",
                elem: "Continue",
            };
        case "delete":
            return {
                color: "danger",
                elem: "Delete",
            };
        case "save":
            return {
                color: "primary",
                elem: "Save",
            };
        case "savechanges":
            return {
                color: "primary",
                elem: "Save changes",
            };
        case "agree":
            return {
                color: "primary",
                elem: "Agree",
            };
        case "disagree":
            return {
                color: "transparent",
                elem: "Disagree",
            };
        case "reject":
            return {
                color: "transparent",
                elem: "Reject",
            };
        case "yesdelete":
            return {
                color: "danger",
                elem: "Yes, delete",
            };
        case "yessave":
            return {
                color: "primary",
                elem: "Yes, save",
            };
        case "yescontinue":
            return {
                color: "primary",
                elem: "Yes, continue",
            };
        case "yesenable":
            return {
                color: "primary",
                elem: "Yes, enable",
            };
        case "nothanks":
            return {
                color: "transparent",
                elem: "No thanks",
            };
    }
};
const genBtn = (customStyle, btn, fn) => {
    let hasDismissButton = false;
    if (btn) {
        if (!Array.isArray(btn)) {
            btn = [btn];
        }
        let aFn = [];
        if (fn) {
            if (!Array.isArray(fn)) {
                aFn = [fn];
            }
            else {
                aFn = fn;
            }
        }
        if (aFn.length < btn.length) {
            hasDismissButton = true;
        }
        let tBtnItem = [];
        for (let x = 0; x < btn.length; x++) {
            let t = btnTypeDB(btn[x]);
            tBtnItem.push({
                color: t.color,
                elem: t.elem,
                click: aFn.length > x ? aFn[x] : undefined,
            });
        }
        return { hasDismissButton: hasDismissButton, btn: genBtnItem(customStyle, tBtnItem) };
    }
    else {
        return { hasDismissButton: false, btn: [] };
    }
};
const Create = (attr) => {
    let contAttr = Object.assign({}, attr);
    delete contAttr.customStyle;
    delete contAttr.btn;
    delete contAttr.btnFn;
    delete contAttr.title;
    delete contAttr.elem;
    delete contAttr.attrHeader;
    delete contAttr.attrBody;
    delete contAttr.attrFooter;
    let btn = genBtn(attr.customStyle, attr.btn, attr.btnFn);
    let showHeader = false;
    let showFooter = btn.btn.length > 0;
    if (!btn.hasDismissButton) {
        contAttr.backdrop ??= "static";
        attr.backdrop ??= "static";
    }
    if (attr.title) {
        showHeader = true;
    }
    else {
        showHeader = !(attr.backdrop === "static" ? false : btn.hasDismissButton);
    }
    if (attr.customStyle === 1) {
        contAttr.contentAttr = { rounded: 4 };
        contAttr.view = "end";
        return new container_js_1.container(contAttr, [
            showHeader
                ? new header_js_1.header((0, mergeAttr_js_1.mergeAttr)({
                    borderNone: "bottom",
                    close: attr.backdrop === "static" ? false : btn.hasDismissButton,
                }, attr.attrHeader), new title_js_1.title(attr.title || document.title))
                : "",
            new body_js_1.body((0, mergeAttr_js_1.mergeAttr)({ paddingY: 0 }, attr.attrBody), attr.elem ? attr.elem : ""),
            showFooter
                ? new footer_js_1.footer((0, mergeAttr_js_1.mergeAttr)({
                    flex: "column",
                    alignItem: "stretch",
                    width: 100,
                    gap: 2,
                    paddingBottom: 3,
                    borderNone: "top",
                }, attr.attrFooter), btn.btn)
                : "",
        ]);
    }
    else if (attr.customStyle === 2) {
        return new container_js_1.container(contAttr, [
            showHeader
                ? new header_js_1.header((0, mergeAttr_js_1.mergeAttr)({
                    textAlign: "center",
                    padding: 4,
                    paddingBottom: 0,
                    borderNone: "bottom",
                    position: "relative",
                    close: false, //manually create float btnclose
                }, attr.attrHeader), [
                    new title_js_1.title({ width: 100 }, attr.title || document.title),
                    (attr.backdrop === "static" ? false : btn.hasDismissButton)
                        ? new btnclose_js_1.btnclose({
                            float: "end",
                            top: 0,
                            end: 0,
                            marginTop: 2,
                            marginEnd: 2,
                            opacity: "25",
                            position: "absolute",
                        })
                        : "",
                ])
                : "",
            new body_js_1.body((0, mergeAttr_js_1.mergeAttr)({ padding: 4, paddingTop: showHeader ? 0 : undefined, textAlign: "center" }, attr.attrBody), attr.elem ? attr.elem : ""),
            showFooter
                ? new footer_js_1.footer((0, mergeAttr_js_1.mergeAttr)({
                    flex: ["nowrap", "grow-1"],
                    padding: 0,
                }, attr.attrFooter), btn.btn)
                : "",
        ]);
    }
    else {
        return new container_js_1.container(contAttr, [
            showHeader
                ? new header_js_1.header((0, mergeAttr_js_1.mergeAttr)({
                    close: attr.backdrop === "static" ? false : btn.hasDismissButton,
                }, attr.attrHeader), new title_js_1.title(attr.title || document.title))
                : "",
            new body_js_1.body(attr.attrBody ? attr.attrBody : {}, attr.elem ? attr.elem : ""),
            showFooter ? new footer_js_1.footer(attr.attrFooter ? attr.attrFooter : {}, btn.btn) : "",
        ]);
    }
};
exports.Create = Create;
