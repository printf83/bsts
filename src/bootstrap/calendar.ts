...
class item extends ul {
	...
	constructor(...arg: tagConstructorArgsNoElement<ICalendar.item>) {
		super(bsConstructorNoElement(arg));
	}

	convert(attr: ICalendar.item) {
		attr.multiple ??= false;
		attr.view ??= new Date();
...
class header extends div {
	...
	constructor(...arg: tagConstructorArgsNoElement<ICalendar.header>) {
		super(bsConstructorNoElement(arg));
	}

	convert(attr: ICalendar.header) {
		attr.view ??= new Date();
		attr.monthTitle ??= [
			"January",
			"February",
										...
										modal.create({
											on: {
												"show.bs.modal": (e) => {
													...
													(
														mdl.querySelector("select[name='month']") as HTMLSelectElement
													).value = view.getMonth().toString();
													(
														mdl.querySelector("input[name='year']") as HTMLInputElement
													).value = view.getFullYear().toString();
												},
											},
											title: "Calendar",
...
export class calendar extends div {
	...
	constructor(...arg: tagConstructorArgsNoElement<ICalendar.container>) {
		super(bsConstructorNoElement(arg));
	}
