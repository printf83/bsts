...
type IBootstrapTypeDisplay =
	| bootstrapType.display[number]
	| bootstrapType.display[number][];
...
const fnRow = (display: IBootstrapTypeDisplay | undefined, elem: IElem) => {
	return new div(
		{ row: true, display: display },
		new div({ col: true, textAlign: "center" }, elem)
	);
};

const fnIcon = (
	display: IBootstrapTypeDisplay | undefined,
	attr: IAttrBSIcon
) => {
	...
	if (attr && typeof attr.icon !== "undefined") {
		if (attr.elem) {
			...
			switch (attr.iconPosition) {
				case "start":
					tElem = new div(
						{ display: "flex", gap: 2, alignItem: "center" },
						[
