...
import { bootstrapAttachRule, bootstrapBase, isBootstrapType } from "../base/bootstrap.js";
const dbRule = {
...
const addBootstrapClass = (key, rule, data, elem) => {
    ...
    if (rule.value && isBootstrapType(data, rule.value)) {
        ...
        if (setting.DEBUG)
            console.warn(`${key}:"${data}" is not supported value for bootstrap property`);
    }
    return elem;
};
export const attachBootstrap = (key, elem, attr) => {
    if (allow(key)) {
...
export const genBootstrapClass = (key, rule, value) => {
    let result = [];
    if (value) {
        //convert to array
        let data = [];
        ...
        data.forEach((d) => {
            ...
            if (rule.value && isBootstrapType(d, rule.value)) {
                ...
                if (setting.DEBUG)
                    console.warn(`${key}:"${d}" is not supported value for bootstrap property`);
            }
        });
    }
    return result;
